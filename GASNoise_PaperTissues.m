function [avgSNRans, contrastans, rippleans, hist_ans, avgans] = GASNoise_PaperTissues(I1, I2, I3, I4, trials, stdev_noise)

ESM = zeros(32,128,trials);
AS = zeros(32,128,trials);
GAS = zeros(32,128,trials);
...
for n = 1:trials
    ...
    noise = normrnd(0, stdev_noise, 32, 128) + 1i * normrnd(0, stdev_noise, 32, 128);
    I4_noise = I4 + noise;
    compdata=zeros(32,128,4,1);
    compdata(:,:,1,1)=I1_noise; 
    compdata(:,:,2,1)=I2_noise; 
    compdata(:,:,3,1)=I3_noise; 
    compdata(:,:,4,1)=I4_noise;
    ...
    GS(:,:,n)=squeeze(abs(GSmaster));
    AS(:,:,n)=squeeze(abs(ASmaster));
    GAS(:,:,n)=squeeze(abs(GASsoln));
