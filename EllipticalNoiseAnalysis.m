...
for alpha = 10:10:90
  ...
  [I1, I2, I3, I4] = SSFP_4PC_NoiseAnalysis(alpha);
  ...
  [avgSNR(1,:), contrast(1,:), ripple(1,:), histogram(1,:,:), avg(1,:)] = CSNoise(I1, I2, I3, I4, trials, stdev_noise);
  disp('Starting Sum-of-Squares Noise Analysis');
  [avgSNR(2,:), contrast(2,:), ripple(2,:), histogram(2,:,:), avg(2,:)] = SOSNoise(I1, I2, I3, I4, trials, stdev_noise);
  disp('Starting Elliptical Signal Model Noise Analysis');
  [avgSNR(3,:), contrast(3,:), ripple(3,:), histogram(3,:,:), avg(3,:)] = ESMNoise(I1, I2, I3, I4, trials, stdev_noise);
  disp('Done!');
  disp(' ');
  
  
  
  save(['EllipticalNoiseAnalysis_FA' num2str(alpha) '_Ratio2to1.mat'],'avgSNR','contrast','ripple','histogram','avg','-v7.3');
